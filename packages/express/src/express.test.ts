...
describe('Express Middleware E2E', () => {
  ...
  it('should not cache requests without idempotency key', async () => {
    ...
    const response1 = await request(app).post('/api/users').send(payload).expect(201);

    const response2 = await request(app).post('/api/users').send(payload).expect(201);
  ...
  it('should use custom error handler for idempotency errors', async () => {
    ...
    customAdapter.get = async () => {
      throw new IdempotencyError(IdempotencyErrorCode.STORAGE_ERROR, 'Storage down');
    };
  ...
  it('should respond with the IdempotencyError status when no handler is configured', async () => {
    ...
    defaultAdapter.get = async () => {
      throw new IdempotencyError(IdempotencyErrorCode.STORAGE_ERROR, 'Storage unavailable');
    };
  ...
  it('should return cached response after waiting for lock', async () => {
    ...
    let getCallCount = 0;
    waitAdapter.get = async (_key: string) => {
      getCallCount++;
      if (getCallCount <= 1) {
        return null;
      }
