...
import { idempotentHandler, createHandlerContext } from '../../src/frameworks/handler.js';
...
describe('idempotentHandler', () => {
  ...
  it('should include body hash in cache key by default', async () => {
    ...
    expect(result2).toBe(20); // Different body = different cache key
  ...
  it('should throw cached error when lock holder stored an error', async () => {
    ...
    adapter.get = async () => {
      getCallCount++;
      if (getCallCount === 1) return null; // First check: cache miss
      return {
  ...
  it('should not include body hash when includeBodyInKey is false', async () => {
    ...
    // Different body should reuse cache key when includeBodyInKey is false
