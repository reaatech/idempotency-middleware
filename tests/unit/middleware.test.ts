...
import { IdempotencyMiddleware } from '../../src/core/IdempotencyMiddleware.js';
...
describe('IdempotencyMiddleware', () => {
  ...
  it('should return cached response on cache hit', async () => {
    ...
    // Populate cache by executing once
  ...
  it('should handle concurrent duplicate requests correctly', async () => {
    ...
    storage.acquireLock = vi.fn(async (key: string, ttl: number) => {
      const result = await originalAcquire(key, ttl);
  ...
  it('should replay errors stored as JSON-serialized payloads', async () => {
    // Simulate a remote adapter where Error objects round-trip through JSON
    // and arrive as plain objects with the @@idempotency/error marker.
  ...
  it('should return 409 conflict when lock holder crashes without storing', async () => {
    // Mock: lock can be acquired, but nothing stored after wait
  ...
  it('should not cache when shouldCache returns false', async () => {
    ...
    const cached = await storage.get(
      expect.stringContaining('no-cache-key') as unknown as string
    );
    // Since we don't know the exact hash, we verify by running again
  ...
  it('should include body hash in cache key by default', async () => {
    ...
    expect(result1).toEqual({ data: 'result' });
    expect(result2).toEqual({ data: 'result' });
    // Different bodies should result in different cache keys, so handler runs twice
  ...
  it('should use vary headers in cache key when configured', async () => {
    ...
    const result1 = await varyMiddleware.execute('vary-key', {
      headers: { 'x-client-version': '1.0' },
    }, handler);

    const result2 = await varyMiddleware.execute('vary-key', {
      headers: { 'x-client-version': '2.0' },
    }, handler);

    expect(result1).toEqual({ data: 'result' });
    expect(result2).toEqual({ data: 'result' });
    // Different vary header values should result in different cache keys
    expect(handler).toHaveBeenCalledTimes(2);
  });
  ...
  it('should extract method and path from context', async () => {
    ...
    const result = await middleware.execute('ctx-key', {
      method: 'PUT',
      path: '/api/items/123',
    }, handler);
  ...
  it('should return double-checked cached response after acquiring lock', async () => {
    ...
    storage.get = vi.fn(async (key: string) => {
      getCallCount++;
      if (getCallCount === 1) return null; // Initial check: miss
      return {
